...
Archon Autopilot starts with a simple boundary: let Qwen do the reading and reasoning,
but keep the consequential decision with a person. For each incoming vendor invoice,
it runs a **bounded multi-step ReAct loop** over **Qwen function-calling**. The agent
**recalls the vendor's history** first, **validates** the invoice, and selects duplicate,
variance, or context checks only when the observed evidence warrants them. Every step
inside that loop is read/analyze-only. The result is **one proposed AP action**, and
then the system stops. **Nothing executes until a human approves the exact
arguments.** It recommends; it never auto-executes.

**The difference from a generic invoice classifier comes down to four things:**
...
1. **A bounded ReAct decision loop over Qwen function-calling.** The agent chooses
   ...
   observations before proposing one action. This is a loop, rather than a single
   fixed prompt.
...
3. **A human-in-the-loop money gate with a *structural* tool-attack defense.** Nothing
   executes until a person approves the exact arguments; the model's tool catalog
   ...
   **autonomously execute by construction**. No detector is assumed perfect; the
   multi-step tool-attack suite tests the structural boundary.
4. **The accounts-payable domain, end to end.** Messy incoming invoice → normalize →
   validate → triage (accrue / pay / query / escalate) → a *proposed* action. This is
   the actual daily work of an AP clerk.

> **Scope and limits.** The decision engine is a **genuine bounded ReAct
...
instead of receiving a payment proposal. The **mechanism is measured as a before/after
behavioural delta**: writeback → recall → the correction appears in the observation
the model reads. The offline escalation is a deterministic policy guard, just as the
offline eval number is; online, `qwen-plus` reasons over the same recalled correction.
See [Learning from corrections](#learning-from-corrections-the-approval-gate-as-a-runtime-correction-signal).
...
1. **Intake:** `POST /intake` with an incoming vendor invoice (structured JSON,
   though fields may be missing, mistyped, or ambiguous because real emails/PDFs are messy).
...
### Two tool tiers: multi-step reasoning with a safe stopping point
...
|---|---|---|
| `recall_vendor_history` | pgvector recall of the vendor's prior invoices; surfaces raw **facts** (prior refs, amounts, dates, the amount ratio) and decides nothing | None |
...
| `request_more_context` | record that more information is needed | None |
...
flowchart TD
    ...
    subgraph LOOP["Bounded multi-step ReAct loop &middot; qwen-plus function-calling"]
      ...
      TERM["Exactly one terminal action<br/>draft_journal_entry &middot; draft_payment<br/>draft_vendor_reply &middot; flag_for_review"]:::terminal
      DEC -->|observe| READ
    ...
    NOTE["Model tool catalog EXCLUDES approve / pay<br/>No injection can autonomously execute"]:::guard
...
Archon Autopilot is deliberately **an autonomous agent embedded in a workflow**. Its
core is a genuine bounded ReAct agent: `qwen-plus` chooses each step and can chain
several read/analyze tools before it acts. That agent's
**consequential edge** is wrapped in a **deterministic, auditable workflow**:
`normalize → agentic decision → human gate → execute`. That is a deliberate design
choice: where money moves, we want **predictability and auditability**, so the
...
is a **second, agent-safe proposal/read surface** over the same core. It is not a
decision surface: approval, amendment, and rejection exist only on authenticated
HTTP/UI.
...
Reproduce or redeploy it with one command on the box. [`deploy/redeploy.sh`](deploy/redeploy.sh)
...
**same Fastify backend**, with no framework, build step, or CDN. It offers:
...
- a **global daily backstop** across all clients (default **2000/day**,
  `UPLOAD_GLOBAL_DAILY_LIMIT`). This is the hard, spoof-proof bound on accepted public
  provider workflows.
...
`qwen-vl-max` is used only when `DASHSCOPE_API_KEY` is set, using the same env-based
...
standard interfaces. These are the custom-skills and MCP integrations called for by
the **Technical Depth & Engineering** criterion.
...
the HTTP routes drive: one decision loop, one memory, and one approval queue, exposed
...
|---|---|---|
| `intake_invoice` | Run the multi-step ReAct loop, persist **PENDING**, and return a least-privilege summary (id/status, proposed tool/confidence, finding counts and trace-tool summary). Invoice, args, rationale, observations and recalled content are redacted by default. Nothing executes. | None |
...
|---|---|---|---|
| `recall_vendor_history` | autonomous | autonomous | None |
...
| `request_more_context` | autonomous | autonomous | None |
| `draft_journal_entry` | terminal | **human-gated** | None |
| `draft_payment` | terminal | **human-gated** | None |
| `draft_vendor_reply` | terminal | **human-gated** | None |
| `flag_for_review` | terminal | **human-gated** | None |
...
actions. This is the exact setup a *multi-step tool-attack* targets: a prompt-injection
...
- **Magic-byte content-sniffing** ([`src/qwen/vision.ts`](src/qwen/vision.ts),
  ...
  content-type (for example, a `.pdf` that is actually a PNG) is rejected with `400`.
  *A full antivirus scan is deliberately out of scope for this demo. This guard only
  checks whether the file is the type it claims to be.*
...
- **Relevance gate** ([`src/qwen/relevance.ts`](src/qwen/relevance.ts),
  ...
  advisory; the human still decides. It does not invoke the decider loop for an
  obviously
...
`event: security` step in the live stream. The approval UI warns that the suspected
instructions are treated as untrusted data and that the human gate still blocks
autonomous execution. This is proven offline by
...
**Playwright e2e** job. All run with no `DASHSCOPE_API_KEY`, so the whole agent runs
...
weighted number** against the live code. Its checks exercise behavior rather than
merely testing whether files exist.
...
docs/video/architecture surface. Checks that need a human with credentials or a
browser, including final playback review, signed-out public-host playback, and
live-box availability, are reported
...
check fails. A regressed check, such as a broken eval, a dropped sink, or an MCP-count drift,
...
tuned, developer-labelled set of **22 AP scenarios** (`eval/dataset.ts`) covers clean new-vendor, clean recurring
...
- **Offline (deterministic Fakes, gated in CI):** **22 / 22 (100.0%)** tuned policy
  ...
  the terminal action. This is a **policy / regression** guard
...
Method, evidence limits, and the offline/online split: [`EVAL.md`](EVAL.md).

---

## Learning from corrections: the approval gate as a runtime correction signal

The human decisions at the approval gate become **feedback the next decision reads**.
When a person **amends** a proposal's amount
...
This is **measured as a behavioural delta**. The same decision
...
> **Scope and limits.** This is a small, deliberately isolated demonstration
> that the gate feedback is *read and changes behaviour*. It rules out a write-only
> interpretation of the memory writeback; it is not model training or a general
> online-learning claim. No model weights are updated. The
...
falls below a threshold (`LOW_CONFIDENCE_THRESHOLD`, default 0.5) receives a
low-confidence review warning in `/pending` (the `lowConfidence` field) and the
...
|---|---|
| **Technical Depth & Engineering (30%)** | A real bounded **multi-step ReAct loop** over `qwen-plus` **function-calling** (`src/ap/loop.ts`) across a two-tier tool set (autonomous read/analyze skills and human-gated terminal actions), with the **same `tool_calls`-parse path online and offline** (a canned `FakeQwenChatClient`). The integration is exercised in CI with no key. The injectable core has two intentionally asymmetric surfaces: HTTP + Approval UI is the exclusive authenticated decision surface, while an agent-safe **four-tool MCP server** can only intake proposals and read queue/memory/catalog state. A derived **nine-skill custom-skills catalog** remains introspectable. Real `qwen-vl-max` document vision on the upload path. Full test pyramid (unit → integration → Playwright e2e) + an **80% coverage gate** + **documentation-drift fitness functions** + gitleaks + dep-audit; live on Alibaba Cloud (ECS + pgvector). |
| **Innovation & AI Creativity (30%)** | **The approval gate creates a runtime correction signal**: a human's amend/reject is written back and *read* on the vendor's next decision, so re-billing an amount a person corrected *down* is escalated instead of receiving a payment proposal (a **measured** before/after behavioural delta; no model weights are updated; see [`EVAL.md`](EVAL.md)). The **structural safety design** keeps approve/pay outside the model's tool catalog, so prompt-injection in untrusted data cannot autonomously execute; the multi-step adversarial suite tests that boundary. Decision quality is a **measured** number (22/22 offline, gated), not asserted. |
...
### Why we deferred an A2A validator-debate layer

We evaluated adding an **agent-to-agent (A2A) "validator debate"** in front of the
gate: a second agent (or a panel) that argues for or against each proposal before it
reaches a human. We deferred it for three reasons:
...
- **The value it targets is already covered**, deterministically: cross-checks
  ...
  scan, and the learning-from-corrections signal. All are inspectable and reproducible
  offline.
...
- **Two real terminal sinks; the rest are simulated adapters.** `draft_vendor_reply`
  ...
  but SMTP cannot guarantee exactly-once delivery to the recipient. When unconfigured,
  both cleanly simulate by recording the intent and writing nothing behind the unchanged
  human gate. A transport-submission or ledger-write failure *propagates* so a failed
  side effect is never silently swallowed. The other two
  (`draft_payment` / `flag_for_review`) still record what *would* happen to inspectable
  in-memory Fakes behind the same `Sinks` interfaces, providing the drop-in seam for a real
  ...
  real**; the email and ledger side effects are real as well.
- **Live on Alibaba Cloud.** The app is deployed on an Alibaba Cloud **ECS** box over
  HTTPS at **https://autopilot.43.106.13.19.sslip.io**, using real Qwen (`qwen-plus` +
