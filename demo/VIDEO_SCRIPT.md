...
| 5 · Correction changes behavior | 1:11–1:34 | Guided synthetic challenge: verified baseline → corrected overbill → matching re-bill/control comparison | Hero innovation: matching re-bill → `flag_for_review`; corrected control → `draft_payment`. Same live routes, no preloaded answer. |
...
python demo/media-tools/add_rights_safe_narration.py --piper-python .artifacts/tts/venv/Scripts/python.exe --replace
...
The base builder internally locks `CAPTION_ONLY=true`: exactly 168 seconds at 30 fps,
burned per-beat captions, a measured nine-cue English SRT, and locally generated
digital silence. The rights-safe narration pass may then replace only that silence
with locally synthesized speech from the pinned public-domain-source voice described
by `demo/media-tools/narration-voice.lock.json`; it never adds music or captured audio.
`demo/media-tools/narration-script.json` is the single text source for burned captions,
the exact accessibility SRT and narration, apart from three guarded phonetic aliases. The final MP4,
