...
must then be consumed by the canonical real-motion builder and the separate
`add_rights_safe_narration.py` pass. Do not use this legacy renderer's historical
network-TTS mode for publication; the canonical pass uses only the pinned local voice.
The manual `Generate Demo Video` workflow remains an intermediate-artifact workflow;
it does not publish the final narrated release.
...
Acceptance is human-owned: verify 1920×1080 H.264/AAC, exact captions, authorized
local narration, no blank lead-in, no credential or stale-count
