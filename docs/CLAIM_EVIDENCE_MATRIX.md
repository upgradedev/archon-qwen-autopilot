...
| Weak extraction fails toward review | `src/ap/extraction-confidence.ts`, `src/ap/normalize.ts`, `src/ap/validate.ts` | `tests/unit/workflow.test.ts`, `tests/unit/normalize.test.ts`, `tests/unit/validate.test.ts` | “Low/unknown confidence, a payable total inferred because the document total was unreadable, missing date/currency, conflicts, or partial line items cannot produce a payment proposal.” Relevance remains advisory. |
