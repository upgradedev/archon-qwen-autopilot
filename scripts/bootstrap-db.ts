...
export function bootstrapConfig(env: NodeJS.ProcessEnv = process.env): BootstrapConfig {
  ...
  return { migrationUrl, runtimeUrl, appPassword, otherDatabase };
}
...
async function withBootstrapLock<T>(config: BootstrapConfig, operation: () => Promise<T>): Promise<T> {
  const lockClient = new Client({
    ...
    application_name: "autopilot-db-bootstrap-lock",
  });
...
async function prepareCluster(config: BootstrapConfig): Promise<void> {
  const admin = new Client({ connectionString: config.migrationUrl, application_name: "autopilot-db-bootstrap" });
  await admin.connect();
  try {
    ...
    if (await roleExists(admin, OTHER_ROLE)) await admin.query(`REVOKE CONNECT ON DATABASE ${APP_DATABASE} FROM ${OTHER_ROLE}`);
    ...
    if (!await roleExists(admin, OTHER_ROLE)) {
      throw new Error("memoryagent_app role does not exist; refusing to modify the Memory database ACL");
    }
    // PostgreSQL privileges are additive: revoking only from autopilot_app would
    // not beat PUBLIC CONNECT. Remove PUBLIC, preserve the fixed Memory app role
    // when present, and deny this app explicitly.
    await admin.query(`REVOKE CONNECT ON DATABASE ${config.otherDatabase} FROM PUBLIC`);
    await admin.query(`REVOKE CONNECT ON DATABASE ${config.otherDatabase} FROM ${APP_ROLE}`);
    await admin.query(`GRANT CONNECT ON DATABASE ${config.otherDatabase} TO ${OTHER_ROLE}`);
  } finally {
    await admin.end();
  }
}
...
async function migrateAndGrant(config: BootstrapConfig): Promise<void> {
  const targetAdmin = new Client({
    connectionString: databaseUrl(config.migrationUrl, APP_DATABASE),
    application_name: "autopilot-db-migration",
  });
  await targetAdmin.connect();
  try {
    const here = dirname(fileURLToPath(import.meta.url));
    const schema = await readFile(join(here, "..", "src", "db", "schema.sql"), "utf8");
    const statements = stripComments(schema).split(";").map((statement) => statement.trim()).filter(Boolean);
    ...
    try {
      ...
      await targetAdmin.query(`GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO ${APP_ROLE}`);
      await targetAdmin.query(`GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO ${APP_ROLE}`);
      ...
      await targetAdmin.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO ${APP_ROLE}`);
      await targetAdmin.query(`ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO ${APP_ROLE}`);
...
async function verifyIsolation(config: BootstrapConfig): Promise<void> {
  const runtime = new Client({ connectionString: config.runtimeUrl, application_name: "autopilot-db-verification" });
  ...
  try {
    ...
    const tablePrivileges = await runtime.query<{
      ...
      [["agent_memory", "ap_workitems", "ap_daily_quota", "ap_process_tickets"]]
    );
    if (tablePrivileges.rows.length !== 4 || tablePrivileges.rows.some((row) =>
      row.owned || !row.can_select || !row.can_insert || !row.can_update || !row.can_delete
      || row.truncate || row.references || row.trigger
    )) {
      throw new Error("autopilot_app table privileges exceed exact DML or table ownership remains");
    }
    ...
    if (sequencePrivileges.rows.some((row) => row.owned || !row.can_use || !row.can_select || row.can_update)) {
      throw new Error("autopilot_app sequence privileges exceed exact USAGE/SELECT or ownership remains");
    }
  ...
  const cross = new Client({ connectionString: databaseUrl(config.runtimeUrl, config.otherDatabase), connectionTimeoutMillis: 5_000 });
  try {
    await cross.connect();
...
async function main(): Promise<void> {
  ...
  console.log("Dedicated autopilot_app role, schema grants and cross-database isolation verified.");
}
