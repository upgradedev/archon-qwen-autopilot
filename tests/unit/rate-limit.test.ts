...
// the default cap is 10, that the (10+1)th consume in a day is rejected without
...
test("the documented default upload cap is 10/day", () => {
  assert.equal(DEFAULT_DAILY_UPLOAD_LIMIT, 10);
  assert.equal(new DailyRateLimiter().snapshot().limit, 10);
});

test("the 11th upload in a UTC day is rejected (and does not increment past the cap)", () => {
  const rl = new DailyRateLimiter(10, () => new Date("2026-07-06T09:00:00Z"));
  for (let i = 1; i <= 10; i++) {
    const r = rl.consume();
    assert.equal(r.allowed, true, `upload #${i} should be allowed`);
    assert.equal(r.used, i);
    assert.equal(r.remaining, 10 - i);
  }
  const eleventh = rl.consume();
  assert.equal(eleventh.allowed, false);
  assert.equal(eleventh.remaining, 0);
  assert.equal(eleventh.used, 10); // still 10 — the cap was not exceeded
  // A 12th attempt is still rejected (idempotent once over).
